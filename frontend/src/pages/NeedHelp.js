...
export default function NeedHelp() {
  ...
  useSocketEvent('help-request:new', (payload) => {
    if (payload.created_by !== userId) return;
    setMyRequests((prev) => [payload, ...prev.filter((item) => item.id !== payload.id)]);
    setStatus({ type: 'success', text: 'Emergency signal sent successfully. Responders have been notified.' });
  });

  useSocketEvent('help-request:claimed', (payload) => {
    if (payload.created_by !== userId) return;
    setMyRequests((prev) => prev.map((item) => (item.id === payload.id ? { ...item, ...payload } : item)));
    setStatus({
      type: 'success',
      text: `Your help request was acknowledged by ${payload.acknowledgedBy?.name || payload.acknowledgedBy?.email || 'a responder'}.`,
    });
  });

  useSocketEvent('help-request:resolved', (payload) => {
    if (payload.created_by !== userId) return;
    setMyRequests((prev) => prev.map((item) => (item.id === payload.id ? { ...item, ...payload, active: false } : item)));
    setStatus({ type: 'success', text: 'Your help request has been marked resolved.' });
  });
  ...
  const latestOpenRequest = useMemo(() => myRequests.find((item) => item.active) || null, [myRequests]);
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <section className="rounded-[30px] border border-white/30 bg-gradient-to-br from-red-700 via-red-600 to-orange-500 p-6 text-white shadow-2xl">
          ...
          <p className="mt-2 max-w-2xl text-sm text-white/85 sm:text-base">
            This sends your live location and your message to police and admin responders immediately.
          </p>
        </section>
        ...
        {latestOpenRequest && (
          <div className="rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-700 shadow-sm">
            Latest active request: {requestStatusLabel(latestOpenRequest)}
            {latestOpenRequest.acknowledgedBy ? ` by ${latestOpenRequest.acknowledgedBy.name || latestOpenRequest.acknowledgedBy.email}` : ''}.
          </div>
        )}
            ...
            <div className="mt-4 rounded-2xl bg-gray-50 p-4">
              {loadingLocation ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-gray-700">Latitude: {location.lat.toFixed(6)}</div>
                  <div className="text-sm text-gray-700">Longitude: {location.lng.toFixed(6)}</div>
                </div>
              ) : (
                <div className="text-sm text-red-600">We could not capture your location. Please allow geolocation and refresh this page.</div>
              )}
            </div>

            <label className="mt-5 block text-sm font-semibold text-gray-700">Help message</label>
            <textarea
              value={message}
          ...
          <div className="space-y-6">
            <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-lg">
              ...
              <div className="mt-4 space-y-3 text-sm text-gray-600">
                ...
                <div className="flex items-start gap-3 rounded-2xl bg-amber-50 p-4">
                  ...
                  <span>You now get live status updates when the request is acknowledged or resolved.</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-lg">
              ...
              <div className="mt-4 space-y-3">
                {myRequests.length === 0 && <div className="rounded-2xl bg-gray-50 p-4 text-sm text-gray-500">No help requests sent yet.</div>}
                ...
                {myRequests.map((request) => (
                  <div key={request.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                    ...
                    <div className="mt-2 text-xs text-gray-500">Sent {request.created_at ? new Date(request.created_at).toLocaleString() : '-'}</div>
                  </div>
                ))}
              </div>
            </div>
