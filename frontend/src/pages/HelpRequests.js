...
export default function HelpRequests() {
  ...
  useSocketEvent('help-request:resolved', (payload) => {
    ...
    setBanner(`Request resolved: ${payload.title}`);
  });

  useSocketEvent('help-request:claimed', (payload) => {
    setRequests((prev) => prev.map((item) => (item.id === payload.id ? { ...item, ...payload } : item)));
    setBanner(`Request claimed by ${payload.acknowledgedBy?.name || payload.acknowledgedBy?.email || 'a responder'}`);
  });
  ...
  const claimRequest = async (id) => {
    try {
      ...
      setRequests((prev) => prev.map((item) => (item.id === id ? { ...item, ...res.data } : item)));
    } catch (err) {
  ...
  return (
    <AppShell userName={getUserName() || 'Responder'}>
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-[28px] border border-gray-100 bg-white p-5 shadow-lg">
          ...
          <div className="mt-5 space-y-4">
            {loading && <div className="rounded-2xl bg-gray-50 p-4 text-sm text-gray-500">Loading help requests...</div>}
              ...
              requests.map((request) => (
                <div key={request.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {request.status === 'claimed' && (
                          <span className="rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-semibold text-indigo-700">Claimed</span>
                        )}
                      </div>
                    ...
                    <div className="text-xs text-gray-500">{request.created_at ? new Date(request.created_at).toLocaleString() : '-'}</div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-xl bg-white p-3">
                      ...
                      <div className="mt-1 text-sm font-semibold text-gray-900 break-all">{request.reporter?.email || '-'}</div>
                    </div>
                    <div className="rounded-xl bg-white p-3">
                    ...
                    <div className="rounded-xl bg-white p-3">
                      ...
                      <div className="mt-1 text-sm font-semibold text-gray-900">
                        {request.latitude && request.longitude ? `${Number(request.latitude).toFixed(5)}, ${Number(request.longitude).toFixed(5)}` : '-'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    {request.active && !request.acknowledgedBy ? (
                      <button
                        type="button"
